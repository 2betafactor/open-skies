...
with tempfile.TemporaryDirectory() as data:
 ...
 with sync_playwright() as p:
  ...
  page.evaluate('''async()=>{const {Journal}=await import('/src/journey.js?v=journey5');const j=new Journal();j.save({kind:'flight',name:'Meadow Airfield',world:'sandbox',start:{name:'Meadow Airfield',lat:0,lng:0},target:{name:'Northern fields',lat:.025,lng:.012},timeSec:30,distanceKm:1,path:[[0,0,7,0],[0,.001,50,0]]});}''')
  ...
  assert json.loads(Path(download.value.path()).read_text())['world']=='sandbox'
  page.get_by_role('button',name='Plan this flight',exact=True).click()
  assert page.locator('[data-world=sandbox]').get_attribute('aria-pressed')=='true'
  ...
  result=page.evaluate('''async()=>{const {Journal}=await import('/src/journey.js?v=journey5');const j=new Journal(),old=Storage.prototype.setItem;let rejected=false;Storage.prototype.setItem=()=>{throw new DOMException('Full','QuotaExceededError')};try{j.save({kind:'flight',name:'Must not save'})}catch{rejected=true}finally{Storage.prototype.setItem=old}return rejected&&j.entries.length===0&&new Journal().entries.length===0;}''')
  assert result
