...
export class Radio {
  ...
  bind(){this.audio.addEventListener('error',()=>{this.status('Nightride FM could not be reached.');});}
  ...
  load(stations,label){this.stations=stations;$('radio-location').textContent=label;}
  play(station){this.station=station;this.audio.src=station.url;this.audio.play().then(()=>{this.status(`Playing ${station.name}.`);}).catch(()=>this.status('Nightride FM is ready when audio is allowed.'));}
}
