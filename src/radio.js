...
export class Radio {
  ...
  async find(){const button=$('radio-locate');button.disabled=true;this.status('Requesting your location…');let pos;try{pos=await new Promise((resolve,reject)=>navigator.geolocation.getCurrentPosition(resolve,reject,{enableHighAccuracy:false,timeout:9000,maximumAge:300000}));}catch{this.load(FALLBACK,'Worldwide picks');button.disabled=false;this.status('Location was unavailable, so here are reliable digital stations.');return;}
    ...
    this.load(stations.length?stations:FALLBACK,stations.length?'Nearby stations':'Worldwide picks');button.disabled=false;this.status(stations.length?'Choose a nearby station to start playback.':'The directory is unavailable, so here are reliable digital stations.');
  }
  load(stations,label){this.stations=stations;const select=$('radio-stations');select.replaceChildren();stations.forEach((s,i)=>{const o=document.createElement('option');o.value=i;o.textContent=`${s.name}${s.country?' · '+s.country:''}${s._distance!=null?' · '+Math.round(s._distance)+' km':''}`;select.append(o);});select.disabled=!stations.length;$('radio-stop').disabled=true;$('radio-location').textContent=label;}
  ...
  stop(){this.audio.pause();this.audio.removeAttribute('src');this.audio.load();$('radio-stop').disabled=true;this.status('Radio stopped. Choose another station when ready.');}
}
